...
const Button: React.FC<ButtonProps> = ({ children, disabled, loading, icon, onlyIcon, justify, ...rest }) => {
  ...
  useEffect(() => {
    ...
    setDisabled(loading ?? false);
  }, [loading]);
  return (
    <button disabled={_disabled}
      className={`bg-[#ff1654] shadow hover:bg-[#ab153d] w-[100%] p-2 min-h-[40px] px-4 rounded text-white flex items-center justify-${justify ?? 'center'} cursor-pointer ${(disabled || _disabled)? 'bg-gray-400 hover:bg-gray-300 cursor-not-allowed' : ''}`}
      {...rest}
    >
